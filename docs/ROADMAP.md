...
Status (**v0.9.2**, 2026-06-20): **Gates 0–5 done; Gate 4 hardware-verified and polished
across many on-panel rounds.** The Map, Timeline, and shared hero/footer render on GxEPD2
in **Public Sans** (nine on-device bitmap weights via `tools/genfonts.sh`), with the
WiFi-join **QR**, the **moon** (almanac convention — shadow inked; illumination/age now
**Meeus-accurate**), **radius-driven** ring labels (fanned), the **monitoring-location**
footer, the **offline** glyph, and the **Change-WiFi confirm**. A **third "Info" view**
(designer variant **B** — clock header + DEVICE table with MAC/IP/version, no web needed)
was added. Settings page reworked (geocode coupled to Save, editable verified location
label). False-Quiet-after-boot bug fixed (re-fetch on clock sync). **Remaining: Gate 1b**
(W5500 Ethernet — board #6348 pending) and the **Gate 6 tail** (watchdog + reset-reason,
72 h soak, recovery card → 1.0.0). Target: built and gifted within ~2 weeks of 2026-06-07.
...
- [x] **Gate 4 — Display integration.** Locked layouts ported to GxEPD2
      (`src/display.cpp`): **B-tight Map** (√-scale rings, radius-driven labels,
      depth-cued dots, double-ring headline, swarm `×n`) + **lollipop Timeline** +
      shared hero + persistent Sky Footer (`●○` indicator, home-pin location,
      sun/moon glyphs) + all states (Connect w/ **QR** + both MACs, Acquiring time,
      Reconnecting, Stale, Quiet, Change-WiFi confirm). **Public Sans** is embedded as
      eight bitmap weights generated by `tools/genfonts.sh` (fontconvert @ DPI 72 so
      the size arg == the spec's CSS-px em). A UTF-8 `·`→drawn-dot shim keeps the
      single-byte font tables small. Footer clock ticks on a **partial refresh** (no
      flash); full renders only on poll / view / connectivity change. **Three views ship
      complete** — Map, Timeline, and an **Info** page (clock + device MAC/IP/version,
      designer variant B); on-panel re-shoot done across many polish rounds. *Open:
      `×n` renders `xN` and the en-dash as `-` (ASCII-only font).* The web mirror remains
      the pixel-twin reference (three-tab toggle). *(needs `pio run -t upload` + eyes on
      the panel.)*
