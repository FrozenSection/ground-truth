...
Status (**v0.8.0**, 2026-06-14): **Gates 0–5 done.** Gate 4 landed — the locked
B-tight Map + lollipop Timeline frames, the shared hero, the persistent Sky Footer,
and every system/alternate state are now rendered on GxEPD2 in **Public Sans**
(eight on-device bitmap weights generated from the TTFs via `tools/genfonts.sh`),
with the WiFi-join **QR**, the scanline-filled **moon**, **radius-driven** ring
labels, the **monitoring-location** footer line, the **offline / reconnecting** glyph,
and the **two-step Change-WiFi confirm**. Both envs compile clean (hw flash ~62%).
**Next: the on-panel re-shoot** — flash v0.8.0 and read the real GDEY042T81 at ~60 cm
to tune the 9–10 px micro-labels, then **Gate 1b** (W5500 Ethernet, board pending) and
the **Gate 6 tail** (watchdog, soak, recovery card). Target: built and gifted within
~2 weeks of 2026-06-07.
...
- [x] **Gate 4 — Display integration.** Locked layouts ported to GxEPD2
      ...
      flash); full renders only on poll / view / connectivity change. **Both pages
      ship complete.** *Open: on-panel re-shoot (legibility of 9–10 px labels);
      `×n` currently renders `xN` and the en-dash as `-` (ASCII-only font).* The web
      mirror remains the pixel-twin reference. *(needs `pio run -t upload` + eyes on
      the panel.)*
